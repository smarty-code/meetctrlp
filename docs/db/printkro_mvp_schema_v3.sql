...
--   9. Printer defaults are stored per printer as JSONB and are never mutated
--      by an individual job.
--  10. A print job stores both partial requested overrides and the fully
--      resolved settings used for execution, preserving reproducibility.
--  11. Printer-specific hardware capability discovery remains relational;
--      print settings are execution instructions and are validated by the
--      application/agent against the discovered printer capabilities.
--  12. Duplex is intentionally not modeled because SIMPLEX is the only
...
-- ============================================================
-- PRINTER PRINT-SETTINGS VALIDATION
-- ============================================================
-- Validates the JSONB shape used by printer presets and print-job overrides.
--
-- allow_partial = FALSE:
--   all six supported settings must be present.
--
-- allow_partial = TRUE:
--   any subset (including {}) is valid, but every supplied key must be valid.
--
-- Supported MVP settings:
--   copies: 1..999
--   orientation: PORTRAIT | LANDSCAPE
--   paperSize: A4 | LETTER | LEGAL | A3 | EXECUTIVE
--   inputTray: AUTO_SELECT | MAIN_TRAY | BYPASS_TRAY | TRAY_1 | TRAY_2 | TRAY_3
--   colorMode: COLOR | GRAYSCALE | MONOCHROME
--   printQualityDpi: DRAFT_300DPI | STANDARD_600DPI | HIGH_1200DPI
--
-- Duplex is intentionally not represented because MVP is simplex-only.
...
BEGIN
    ...
    -- Reject unknown keys so the stored contract stays deterministic.
...
-- PRINTERS
-- ============================================================
-- Represents an actual printer discovered/configured by a Print Agent.
--
-- default_print_settings is the printer's persistent preset. It is the
-- baseline configuration for jobs routed to this printer.
--
-- A job MUST NOT update this column when applying per-job overrides.
-- Instead:
--
--   resolved_settings = default_print_settings || requested_overrides
--
-- The resolved object is copied into print_jobs.resolved_settings so a
-- historical job remains reproducible even if the printer preset changes.
...
CREATE TABLE printers (
    ...
    -- Persistent printer-level preset.
    -- Duplex is intentionally absent: MVP supports simplex only.
...
--
-- requested_overrides contains ONLY fields explicitly supplied for this
-- job. It does not contain the printer's default values.
--
-- resolved_settings is the immutable execution snapshot:
--
...
--   resolved_settings
--
-- The application/agent must validate the resolved settings against the
-- printer's capabilities before execution.
...
CREATE TABLE print_jobs (
    ...
    -- Partial per-job printer-setting overrides.
    requested_overrides JSONB NOT NULL DEFAULT '{}'::jsonb,

    -- Fully resolved settings actually selected for this job.
...
-- ============================================================
-- PRINT PRESET / JOB OVERRIDE SEMANTICS
-- ============================================================
-- The application MUST resolve settings atomically when creating a job:
--
--   1. Lock/read the selected printer row.
--   2. Read default_print_settings.
--   3. Validate requested_overrides.
--   4. Resolve with JSONB shallow merge:
--          default_print_settings || requested_overrides
--   5. Validate the resolved object.
--   6. Validate resolved settings against printer_capabilities and the
--      document/order requirements.
--   7. Insert print_jobs with BOTH requested_overrides and resolved_settings.
--
-- Never update printers.default_print_settings as part of job creation.
-- Updating a preset affects only future jobs.
--
-- A job's resolved_settings is the execution snapshot and must be treated
-- as immutable after execution begins. If a job is retried on another
-- printer, create/update the execution assignment according to the
-- application's retry policy and revalidate the settings against the new
-- printer. Do not silently change the stored snapshot without an audit
-- record.
