...
> **Source of truth:** `printkro_mvp_schema_v2.sql`
>
...
Represents a physical printer known to a Print Agent.

A printer is the execution target for a print job.

### Columns

| Column | Type | Null | Default | Purpose |
|---|---|---:|---|---|
| `id` | `UUID` | No | UUID generation | Printer identity. |
| `shop_id` | `UUID` | No | — | Shop that owns/uses the printer. |
| `print_agent_id` | `UUID` | No | — | Agent through which this printer is managed. |
| `name` | `VARCHAR(150)` | No | — | PrintKro-friendly display name. |
| `system_name` | `VARCHAR(255)` | No | — | Printer name as exposed by the local OS/printing subsystem. |
| `manufacturer` | `VARCHAR(150)` | Yes | — | Manufacturer, when detected. |
| `model` | `VARCHAR(150)` | Yes | — | Printer model, when detected. |
| `status` | `printer_status` | No | `OFFLINE` | Current printer status. |
| `is_default` | `BOOLEAN` | No | `FALSE` | Whether printer is configured as the shop's default printer. |
| `last_seen_at` | `TIMESTAMPTZ` | Yes | — | Last observed/heartbeat time. |
| `created_at` | `TIMESTAMPTZ` | No | `now()` | Creation time. |
| `updated_at` | `TIMESTAMPTZ` | No | `now()` | Last modification time. |
...
This prevents the same agent from registering the same OS printer name twice.

### Important application rule

The schema does not ensure that:
...
```text
print_agents.shop_id
```

because there is no composite foreign key enforcing that invariant.

The Print Agent registration service must validate it.
...
This is intentionally separate from the business `orders` table.

An order is what the customer purchased.

A print job is what the physical print system needs to execute.

### Columns

| Column | Type | Null | Default | Purpose |
|---|---|---:|---|---|
| `id` | `UUID` | No | UUID generation | Print job identity. |
| `order_id` | `UUID` | No | — | Parent business order. |
| `order_document_id` | `UUID` | No | — | Document being printed. |
| `printer_id` | `UUID` | Yes | — | Selected physical printer. Nullable while routing/assignment is pending. |
| `print_agent_id` | `UUID` | Yes | — | Agent responsible for execution. Nullable before assignment. |
| `status` | `print_job_status` | No | `QUEUED` | Physical execution lifecycle state. |
| `priority` | `INTEGER` | No | `0` | Queue priority. Higher values sort first in the queue index. |
| `retry_count` | `INTEGER` | No | `0` | Number of retry attempts. |
| `queued_at` | `TIMESTAMPTZ` | No | `now()` | Time job entered queue. |
...
Printer and agent references use `SET NULL`.

This is deliberate:

- historical print jobs should not disappear because a printer is removed;
- the job can retain its historical record even if the physical device is no longer registered.

### Important relationship

An order may have multiple print jobs.

Example:

```text
Order
 ├── Document A → Print Job → Printer 1
 ├── Document B → Print Job → Printer 1
 └── Document C → Print Job → Printer 2
```
...
matches the order referenced by:
...
supports retrieving work in a priority/FIFO-like order.
...
This document is intentionally organized around the actual objects in `printkro_mvp_schema_v2.sql`. The field-level sections explain the purpose and engineering meaning of each column in every table, while the ENUM, constraint, index, trigger, relationship, delete-behavior, workflow, security, concurrency, and implementation sections explain how those fields should be used.
