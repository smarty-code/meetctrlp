...
> **Theme:** dark — Duolingo-style dark learning canvas with high-contrast content and tactile controls

The dark mode is an additive layer. **All light-mode values and component guidance above remain unchanged.** Dark mode changes only semantic surfaces, text, borders, states, and the treatment of interactive controls.

## Dark-mode visual direction

The supplied Duolingo screenshots establish a very dark blue-green canvas rather than pure black. Content sits directly on the canvas or on slightly lighter blue-gray surfaces. Structural borders are visible but subdued. Primary text is near-white; secondary labels are cool gray-blue. The signature Duolingo green remains highly saturated for primary actions, progress, and positive emphasis. Cyan-blue is used for selected/secondary states. Purple may be used only for feature-specific promotional surfaces such as Quests.

### Dark palette

| Semantic role | Value | Token |
|---|---|---|
| App canvas | `#111E22` | `--dark-surface-app` |
| Raised surface | `#1B2B31` | `--dark-surface-raised` |
| Elevated surface | `#22343B` | `--dark-surface-elevated` |
| Pressed surface | `#2A3D45` | `--dark-surface-pressed` |
| Primary text | `#F7F9FA` | `--dark-text-primary` |
| Secondary text | `#9AAAB2` | `--dark-text-secondary` |
| Tertiary text | `#6F8089` | `--dark-text-tertiary` |
| Structural border | `#354852` | `--dark-border-strong` |
| Subtle border | `#273940` | `--dark-border-subtle` |
| Primary action | `#93DE28` | `--dark-action-primary` |
| Primary action pressed | `#7CC01F` | `--dark-action-primary-pressed` |
| Primary action label | `#132126` | `--dark-action-primary-text` |
| Secondary action | `#49B5EF` | `--dark-action-secondary` |
| Secondary selected surface | `#203640` | `--dark-action-secondary-surface` |
| Progress track | `#354852` | `--dark-progress-track` |
| Progress fill | `#8DDB27` | `--dark-progress-fill` |
| Disabled surface | `#3A4A53` | `--dark-state-disabled` |
| Disabled text | `#6B7B84` | `--dark-state-disabled-text` |
| Quest promotional surface | `#8C62C9` | `--dark-quest-purple` |

These values are deliberately semantic rather than replacements for the original light primitives. This prevents light mode from changing and lets components consume the same semantic role in either theme.

## Dark-mode surfaces

- The app/page canvas uses `--dark-surface-app`.
- Cards, option rows, dialogs, and bottom sheets use `--dark-surface-raised`.
- Selected options use `--dark-action-secondary-surface` with a `2px` `--dark-action-secondary` border.
- Progress tracks use `--dark-progress-track`; active progress uses `--dark-progress-fill`.
- Do not use pure black as the default page background.
- Do not introduce gradients.
- Do not add blurred drop shadows. Preserve the existing flat, border-based depth language.

## Dark-mode typography

Keep the existing `din-round` and `feather` families, type scale, weights, tracking, and line heights exactly as defined above. Only color changes:

- Primary UI copy: `--dark-text-primary`
- Secondary/helper copy: `--dark-text-secondary`
- Disabled/caption copy: `--dark-text-tertiary`
- Large brand/display emphasis: `--dark-action-primary`
- Secondary emphasis/selected text: `--dark-action-secondary`

The screenshots use bright, soft-white text instead of the light-mode dark gray. Avoid pure black text on dark surfaces.

## Dark-mode components

### Primary Filled Button

Use `--dark-action-primary` for the fill and `--dark-action-primary-text` for the label. Keep the existing `12px` radius and pressable bottom-edge treatment. The bottom edge should be a darker solid green (`--dark-action-primary-pressed`), not a shadow.

### Ghost / Outlined Secondary Button

Use the dark canvas or raised surface as the background, `--dark-action-secondary` for the `2px` border and label. For green secondary actions, use the existing brand green semantic accent where appropriate, but do not fill the control with `--color-lingot-lime`.

### Option / List Row

Use `--dark-surface-raised` with a `2px` `--dark-border-strong` outline. A selected row uses `--dark-action-secondary-surface` and `--dark-action-secondary`. Unselected rows remain dark and quiet. This is the characteristic treatment visible in the language-selection and placement screens.

### Top Progress Bar

Use `--dark-progress-track` for the track and `--dark-progress-fill` for completed progress. Keep the bar flat and rounded; no glow or gradient.

### Navigation / Bottom Navigation

Use `--dark-surface-app` as the base. Selected navigation controls can use a subtle `--dark-action-secondary-surface` container and `--dark-action-secondary` outline. Unselected icons/text use `--dark-text-secondary` or `--dark-text-tertiary`.

### Quest / Promotional Header

A purple promotional panel is allowed as a **feature-specific semantic surface** (`--dark-quest-purple`). It is not a replacement for the core brand palette and should not become a general-purpose accent.

## Dark-mode states

| State | Surface | Border | Text |
|---|---|---|---|
| Default | `--dark-surface-raised` | `--dark-border-strong` | `--dark-text-primary` |
| Hover | `--dark-surface-elevated` | `--dark-border-strong` | `--dark-text-primary` |
| Selected | `--dark-action-secondary-surface` | `--dark-action-secondary` | `--dark-action-secondary` |
| Focus | `--dark-surface-elevated` | `--dark-action-secondary` | `--dark-text-primary` |
| Disabled | `--dark-state-disabled` | `--dark-border-subtle` | `--dark-state-disabled-text` |
| Primary CTA | `--dark-action-primary` | `--dark-action-primary-pressed` | `--dark-action-primary-text` |

## Theme activation

The CSS extension supports either of these activation strategies:
...
The semantic variables are scoped to both selectors. A `prefers-color-scheme: dark` fallback is intentionally **not** enabled automatically, so an app can keep explicit control over its theme and preserve the existing light mode.

## Dark-mode implementation rule

Do not rewrite or reinterpret the original light-mode primitives. Components should consume semantic roles such as `--surface-app`, `--surface-raised`, `--text-primary`, `--border-strong`, and `--action-primary`; those roles resolve to the light palette by default and to the dark palette under `[data-theme="dark"]` or `.dark`.
