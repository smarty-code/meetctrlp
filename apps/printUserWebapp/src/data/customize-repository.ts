...
export const customizeCopy = {
  ...
  pageError: (pageCount: number) => `Use valid pages from 1 to ${pageCount}, such as 1,3,5-8.`,
...
export const mockConfigurationDocuments: ConfigurableDocument[] = [
  createMockDocument('demo-resume', 'Resume.pdf', 245760, 'application/pdf', customizeConfig.defaultPageCount),
  createMockDocument('demo-notes', 'Lecture-notes.pdf', 524288, 'application/pdf', 8),
];

function createMockDocument(id: string, name: string, size: number, type: string, pageCount: number): ConfigurableDocument {
  return { id, name, size, type, pageCount, status: 'ready', configuration: cloneConfiguration(defaultPrintConfiguration) };
}

export function cloneConfiguration(configuration: PrintConfiguration): PrintConfiguration {
  return { ...configuration, pageSelection: { ...configuration.pageSelection, pages: [...configuration.pageSelection.pages] } };
}