...
interface PageSelectionControlProps { mode: PageSelectionMode; expression: string; pageCount: number; onChange: (mode: PageSelectionMode, expression: string) => void; isValid: boolean; }

export function PageSelectionControl({ mode, expression, pageCount, onChange, isValid }: PageSelectionControlProps) {
  return (
    <fieldset className="rounded-xl border border-graphite/20 bg-paper p-4 sm:p-5 lg:col-span-2 shadow-xs">
      <legend className="px-1 text-[15px] font-bold text-midnight">{customizeCopy.pageSelectionLegend}</legend>
      <div className="mt-3 flex flex-wrap gap-3">
        {(['all', 'selected'] as const).map((option) => (
          ...
          >
            {option === 'all' ? customizeCopy.allPages : customizeCopy.selectedPages}
          </button>
        ))}
      </div>
      {mode === 'selected' && (
        <div className="mt-4">
          <label htmlFor="page-expression" className="text-caption font-bold text-ash">
