        ...
        {(["portrait", "landscape"] as const).map((mode) => (
          ...
          >
            <div className="flex min-w-0 items-center gap-2.5">
              {mode === "portrait" ? (
                <RectangleVertical className="size-5 shrink-0 stroke-[2.2] text-macaw-blue" />
              ) : (
                <RectangleHorizontal className="size-5 shrink-0 stroke-[2.2] text-macaw-blue" />
              )}
              <span className="min-w-0 truncate">
                {mode === "portrait"
                  ? customizeCopy.portrait
                  : customizeCopy.landscape}
                <small className="block text-caption font-medium text-ash">
                  {mode === "portrait"
                    ? customizeCopy.portraitHint
                    : customizeCopy.landscapeHint}
                </small>
              </span>
            </div>
            {value === mode && (
